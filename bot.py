...
async def callback_rename_sound(callback: CallbackQuery):
    ...
    await callback.message.answer("✏️ Пришли новое имя для звука:")
    await callback.answer()
...
async def callback_last_videos(callback: CallbackQuery):
    ...
    # берём последние 5 видео по истории
...
# === Запуск бота ===
async def main():
    asyncio.create_task(check_new_videos())  # уведомления о новых видео
