...
def build_sounds_keyboard(page: int = 0):
    ...
    for i, sound in enumerate(sounds_page, start=start):
        inline_keyboard.append([
            ...
            InlineKeyboardButton(
                text=f"🎬 История звуков",
                callback_data=f"history_sound_{i}"
            )
        ])
...
@dp.message(AddSoundStates.waiting_for_name)
async def rename_sound_get_name(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if user_id in rename_state:
        idx = rename_state.pop(user_id)
        SOUND_URLS[idx]['name'] = message.text if message.text.lower() != 'нет' else None
        with open(SOUNDS_FILE, 'w') as f:
            json.dump(SOUND_URLS, f)
        await message.answer(f"✅ Звук переименован: {SOUND_URLS[idx].get('name') or SOUND_URLS[idx]['url']}", reply_markup=get_main_keyboard())
    await state.clear()

# === CALLBACK: История звуков (2 варианта) ===
@dp.callback_query(lambda c: c.data.startswith("history_sound_"))
async def callback_history_sound(callback: CallbackQuery):
    idx = int(callback.data.split("_")[-1])
    if 0 <= idx < len(SOUND_URLS):
        sound_url = SOUND_URLS[idx]['url']
        last_parser = seen_videos.get(sound_url, [])[-5:]  # последние из парсера

        # Собираем текст для сообщения
        text = f"🎬 История звуков для {SOUND_URLS[idx].get('name') or 'Без имени'}:\n\n"
        text += "🟢 Последние 5 видео по парсеру:\n"
        if last_parser:
            for i, v in enumerate(reversed(last_parser), start=1):
                text += f"{i}. {v}\n"
        else:
            text += "Нет видео\n"
